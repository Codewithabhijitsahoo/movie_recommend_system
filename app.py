...
def fetch_movie_details(movie_id):
    try:
        api_key = "9578fe9fcfa44dcafb9cd5ba675ec208"
        ...
        rating = data.get('vote_average', 0) / 2  # Convert to 5-star scale
        return poster, genres, overview, rating
    except Exception as e:
        st.error(f"Error fetching details: {e}")
        return "https://via.placeholder.com/500x750?text=No+Image", [], "No description.", 0

def recommend(movie, num_recommendations=10):
    movie_index = movies[movies['title'] == movie].index[0]
    distances = similarity[movie_index]
    movie_list = sorted(list(enumerate(distances)), reverse=True, key=lambda x: x[1])[1:1 + num_recommendations]
    ...
    for i in movie_list:
        ...
        poster, genres, overview, rating = fetch_movie_details(movie_id)
